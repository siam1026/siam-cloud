...
import com.siam.package_user.mapper.internal.VipRechargeRecordMapper;
...
public class VipRechargeRecordServiceImpl implements VipRechargeRecordService {
    ...
    public void updateByPayNotify(String outTradeNo) {
        ...
        Member dbMember = memberFeignApi.selectByPrimaryKey(dbVipRechargeRecord.getMemberId()).getData();
        if(dbMember == null){
        ...
        if(dbMember.getType().equals(Quantity.INT_1)){
            ...
            dbMember = memberFeignApi.selectByPrimaryKey(dbVipRechargeRecord.getMemberId()).getData();
        }
        ...
        if(dbVipRechargeDenomination.getIsGiveCoupons()){
            ...
            for (VipRechargeDenominationCouponsRelation denominationCouponsRelation : denominationCouponsRelationList) {
                //获取优惠卷
                Coupons coupons= couponsFeignApi.selectByPrimaryKey(denominationCouponsRelation.getCouponsId()).getData();
