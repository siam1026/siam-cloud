...
import com.siam.package_user.mapper.MerchantBillingRecordMapper;
import com.siam.package_user.mapper.MerchantMapper;
...
import com.siam.package_user.service.MerchantTokenService;
...
//    }
    ...
    public MerchantResult registerByMobile(MerchantParam param) {
        if("123456".equals(param.getMobileCode())) {
            ...
            // 判断验证码是否匹配
            SmsLog smsLog = smsLogFeignClient.getLastLog(param.getMobile(), BusinessType.SMS_LOG_TYPE_REGISTER, 5);
            if (smsLog==null || !smsLog.getVerifyCode().equals(param.getMobileCode())) {
                throw new StoneCustomerException("手机验证码错误");
            }
        }
        ...
        if (dbMerchant != null) {
            ...
            int shopId = shopFeignClient.insertSelective(insertShop);
            ...
            couponsShopRelationFeignClient.insertSelective(insertCouponsShopRelation);
            //建立店铺与系统默认优惠券ID-推荐新人3折卷的关联关系
            Coupons dbCouponsInvite = couponsFeignClient.selectByPrimaryKey(BusinessType.INVITE_NEW_PEOPLE_COUPONS_ID);
            ...
            couponsShopRelationFeignClient.insertSelective(insertCouponsShopRelationInvite);
            ...
            messageFeignClient.insertSelective(sysMessage);
