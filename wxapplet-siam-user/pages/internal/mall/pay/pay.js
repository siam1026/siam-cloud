...
Page({
  ...
  close(){
    ...
    var _this=this;
    https.request('/api-order/rest/member/pointsMall/order/insert', _this.data.isPayJson).then(result => {
      toastService.hideLoading();
      if (result.success) {
  ...
  createOrderToken: function (e) {
    toastService.showLoading();
    https.request('/api-order/rest/member/pointsMall/order/createOrderToken', {}).then(result => {
      if (result.success) {
  ...
  getCouponsMemberRelation(prePageData) {
    ...
    toastService.showLoading();
    https.request('/api-goods/rest/member/pointsMall/couponsMemberRelation/list', {
      pageNo: -1,
      pageSize: 20,
      isUsed: 0,
      isExpired: 0,
  ...
  selectCommissionReward(){
    https.request('/api-order/rest/member/pointsMall/order/selectCommissionReward',{
  ...
  weChatPayTap() {
    ...
    if(this.data.userInfo.type==1){
      ...
      toastService.showLoading();
      https.request('/api-order/rest/member/pointsMall/order/insert', data).then(result => {
        toastService.hideLoading();
        if (result.success) {
          // toastService.showLoading("正在支付...", true);
          //console.log(result.data)
