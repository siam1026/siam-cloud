...
Page({
  ...
  getShoppingCartList() {
    https.request('/api-goods/rest/member/pointsMall/shoppingCart/list', {
      pageNo: -1,
      pageSize: 20
    }).then(result => {
      if (result.success) {
  ...
  getLoveIt() {
    toastService.showLoading();
    https.request('/api-goods/rest/pointsMall/goods/guessLikeGoodsList', {}).then(result => {
      toastService.hideLoading();
      if (result.success) {
  ...
  updateNumber(id, number, type, callbak) {
    https.request('/api-goods/rest/member/pointsMall/shoppingCart/updateNumber', {
      id: id,
      number: number,
      type: type
    }).then(result => {
