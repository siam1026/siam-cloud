...
Page({
   ...
   openConfirm() {
      ...
      if (this.data.order.actualPrice > this.data.userInfo.points) {
         paymentModes[2].desc = "积分不足";
   ...
   inputPwd: function (e) {
      ...
      if (e.detail.value.length >= 6) {
         ...
         console.log(this.data.paymentModes[this.data.paymentModeIndex].value)
         if (this.data.paymentModeIndex == 1) {
            this.balancePay();
         }
         if (this.data.paymentModeIndex == 2) {
            this.pointsPay();
         }
      }
   },
   ...
   pointsPay() {
      ...
      https.request('/api-order/rest/member/platformPay/byPoints', {
         openid: this.data.balanceOpenId,
         type: 4,
         paymentMode: 3,
         paymentPassword: password,
   ...
   cancelOrder() {
      ...
      toastService.showModal(null, "确定取消这个订单吗?", function () {
         https.request('/api-order/rest/member/pointsMall/order/cancelOrder',{
            id:id
         }).then(result => {
            if (result.success) {
   ...
   applyRefund(e) {
      ...
      wx.navigateTo({
         url: '../refund/apply/apply?orderId=' + this.data.order.id+'&type='+e.currentTarget.dataset.type,
      })
   },
   applyServer(){
      this.setData({
   ...
   getOrderDetail(id) {
      toastService.showLoading();
      https.request('/api-order/rest/member/pointsMall/order/selectById', {
         id: id
      }).then(result => {
         toastService.hideLoading();
         if (result.success) {
   ...
   contactBussinessTap() {
      wx.makePhoneCall({
         phoneNumber: this.data.current.customerServicePhone
      }).catch((e)=>{
   ...
   selectLogistics(e) {
      let logisticsJson={};
      logisticsJson.logisticsNo=this.data.order.logisticsNo;
      logisticsJson.logisticsWay=this.data.order.logisticsWay;
      logisticsJson.courierName=this.data.order.courierName;
      logisticsJson.courierPhone=this.data.order.courierPhone;
      logisticsJson.deliveryStatus=this.data.order.deliveryStatus;
      logisticsJson.isSign=this.data.order.isSign;
      logisticsJson.deliveryLastUpdateTime=this.data.order.deliveryLastUpdateTime;
      logisticsJson.takeTime=this.data.order.takeTime;
      logisticsJson.orderId=this.data.order.id;
      wx.navigateTo({
