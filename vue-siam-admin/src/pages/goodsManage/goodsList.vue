<template>
	<section>
      ...
      <!-- <el-table-column prop="exchangePoints" label="兑换商品所需积分数量">
				<template scope="scope">
          <span>{{scope.row.exchangePoints}}份</span>
        </template>            
      </el-table-column> -->
	...
	export default {
      ...
      removeSpecification(item) {
        ...
        // alert(JSON.stringify(item));
				vue.$http.post(vue, '/rest/admin/goodsSpecificationOption/delete', {"ids": [data.id]},
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
      ...
      addSpecification(row) { // 新增规格
        ...
        let url = ''
        param.id ? url = '/rest/admin/goodsSpecificationOption/update' : url = '/rest/admin/goodsSpecificationOption/insert'
          ...
          $.ajax({
			...
			handleDel (id) { // 删除
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					// this.listLoading = true;
				let vue = this;
				vue.$http.post(vue, '/rest/admin/goods/delete', {"id":id},
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
