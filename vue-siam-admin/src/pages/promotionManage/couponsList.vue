<template>
	<section>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column label="操作" fixed="right" width="260">
				<template slot-scope="scope">
					<el-button size="small" @click="handleRelationShop(scope.row.id)">关联店铺</el-button>
		...
		<!--关联店铺界面-->
		<el-dialog title="关联店铺" :visible.sync="editFormVisibleShopTable" @close="closeDialog" :close-on-click-modal="false">
      		<div class="dialogDiv">		
...
<script>
	export default {
		...
		methods: {
			...
			editSubmitShopTable: function () { // 关联店铺保存事件
				let vue = this
				let param = {
					couponsId : this.couponsIdShopTable,
					shopIdListStr : ''
				};
				let url = '';
