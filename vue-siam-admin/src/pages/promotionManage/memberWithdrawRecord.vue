		...
		methods: {
			...
			handlePay (id) { // 打款
				this.$confirm('确认同意并打款吗?', '提示', {
					...
					param.id = id;
					param.status = 1;	
					let url = '/api-user/rest/admin/memberWithdrawRecord/auditApplyWithdraw';
				...
				vue.$http.post(vue, '/api-user/rest/admin/memberWithdrawRecord/list', param,
					(vue, data) => {
				...
				vue.$http.post(vue, '/api-user/rest/admin/memberWithdrawRecord/statisticalAmount', param,
					(vue, data) => {
						//统计提现成功金额						
			...
			handleDel (id) { // 删除
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					vue.$http.delete(vue, '/api-user/rest/admin/memberWithdrawRecord/delete', {"ids" : [id]},
						function(vue, data) {
							vue.$message({
								showClose: true,
								message: data.message,
