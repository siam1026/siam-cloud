	...
	export default {
		...
		methods: {
			...
			getList(pageNoParam) { // 获取列表
				...
				vue.$http.post(vue, '/api-order/rest/admin/appraise/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					let vue = this;
					vue.$http.delete(vue, '/api-order/rest/admin/appraise/delete', {"id" : id},
						function(vue, data) {
							vue.$message({
								showClose: true,
								message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '/api-order/rest/admin/reply/insert';
