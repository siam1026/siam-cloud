...
import com.siam.package_goods.model.param.SearchParam;
import com.siam.package_merchant.entity.Shop;
import com.siam.package_merchant.model.example.ShopExample;
import com.siam.package_merchant.model.vo.ShopAdditionalVo;
import com.siam.package_merchant.model.vo.ShopDetailVo;
...
public class ShopController {
    ...
    public BasicResult list(@RequestBody @Validated(value = {}) Shop shop){
        ...
        page.getRecords().forEach(map -> {
            ...
            //统计月销量
            int latelyMonthlySales = orderFeignApi.selectLatelyMonthlySalesByShopId(startTime, endTime, (int) map.get("id")).getData();

            //计算配送时长、距离公里数
            /*String addressB = (String) map.get("province") + map.get("city") + map.get("area") + map.get("street");*/
            TravelingDistanceVo travelingDistanceVo = commonFeignApi.selectTravelingDistance(coordinateMap.get("lng"), coordinateMap.get("lat"), (BigDecimal) map.get("longitude"), (BigDecimal) map.get("latitude")).getData();
            ...
            List<FullReductionRule> fullReductionRuleList = fullReductionRuleFeignApi.selectByShopId(fullReductionRule).getData();
    ...
    public BasicResult search(@RequestBody @Validated(value = {}) SearchParam searchParam){
        ...
        if(!filterList1.isEmpty()){
            goodsParam.setFilterList1(filterList1);
            goodsParam.setName("%"+ searchParam.getKeywords() +"%");
        }else{
            goodsParam.setName("%"+ searchParam.getKeywords() +"%");
        }
        ...
        if(matchShopIdList.size() > 0){
            ...
            page.getRecords().forEach(map -> {
                ...
                //查询店铺内匹配的商品
                GoodsParam goodsExampleTemp = new GoodsParam();
                goodsExampleTemp.setShopId((int) map.get("id"));
                goodsExampleTemp.setName("%"+ searchParam.getKeywords() +"%");
                List<Goods> goodsListTemp = goodsFeignApi.selectByExample(goodsExampleTemp).getData();
