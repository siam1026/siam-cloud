...
import com.siam.package_goods.model.vo.PromotionVo;
import com.siam.package_goods.model.vo.ShopAdditionalVo;
import com.siam.package_goods.model.vo.ShopDetailVo;
...
public class ShopController {
    ...
    public BasicResult list(@RequestBody @Validated(value = {}) Shop shop){
        ...
        page.getRecords().forEach(map -> {
            ...
            //统计月销量
            int latelyMonthlySales = orderFeignClient.selectLatelyMonthlySalesByShopId(startTime, endTime, (int) map.get("id"));

            //计算配送时长、距离公里数
            /*String addressB = (String) map.get("province") + map.get("city") + map.get("area") + map.get("street");*/
            TravelingDistanceVo travelingDistanceVo = commonFeignClient.selectTravelingDistance(coordinateMap.get("lng"), coordinateMap.get("lat"), (BigDecimal) map.get("longitude"), (BigDecimal) map.get("latitude"));
