<template>
	<section>
		...
		<el-col :span="24" class="toolbar">
			<!-- <el-button type="danger" @click="batchRemove" :status="this.sels.length===0">批量删除</el-button> -->
	...
	export default {
		...
		methods: {
			...
			handleCurrentChange(val) {
				...
				vue.$http.post(vue, '/api-goods/rest/merchant/coupons/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					vue.$http.delete(vue, '/api-goods/rest/merchant/coupons/delete', {"id" : id},
						function(vue, data) {
							vue.$message({
								showClose: true,
								message: data.message,
			...
			handleGiveCoupons (id) { // 派发优惠券
					let vue = this;
					vue.$http.post(vue, '/api-goods/rest/merchant/couponsMemberRelation/giveCoupons', {"couponsId" : id},
						function(vue, data) {
							vue.$message({
								showClose: true,
								message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {						
						...
						if(param.id){
							url = '/api-goods/rest/merchant/coupons/update';
							...
							url = '/api-goods/rest/merchant/coupons/insert';
							vue.$http.post(vue, url, param,
								(vue, data) => {
									// this.editLoading = false;
									vue.$message({
			...
			getListGoodsTable() { // 获取商品列表
				...
				vue.$http.post(vue, '/api-goods/rest/merchant/goods/list', param,
					(vue, data) => {
			...
			handleRelationGoods (id) { // 关联商品
				...
				let vue = this
				vue.$http.post(vue, '/api-goods/rest/merchant/coupons/selectById', {"id" : id},
					(vue, data) => {
			...
			editSubmitGoodsTable: function () { // 关联商品保存事件
				...
				url = '/api-goods/rest/merchant/couponsGoodsRelation/insert';
				vue.$http.post(vue, url, param,
					(vue, data) => {
						// this.editLoadingGoodsTable = false;
						vue.$message({
