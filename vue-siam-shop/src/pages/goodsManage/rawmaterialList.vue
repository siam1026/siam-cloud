		...
		methods: {
			...
			handleDel (id) { // 删除
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					vue.$http.delete(vue, '/api-goods/rest/merchant/rawmaterial/delete', {"id" : id},
						function(vue, data) {
							vue.$message({
								showClose: true,
								message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						if(param.id){
							url = '/api-goods/rest/merchant/rawmaterial/update';
							...
							url = '/api-goods/rest/merchant/rawmaterial/insert';
