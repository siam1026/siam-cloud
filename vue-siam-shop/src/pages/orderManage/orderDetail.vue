	...
	export default {
		...
		methods: {
			...
			printOrderDetail() {				
				...
				//获取订单信息
				let vue = this
				vue.$http.post(vue, '/api-order/rest/merchant/order/selectById', {"id" : id},
					(vue, data) => {
						let order = data.data;
						...
						//获取订单明细信息
						vue.$http.post(vue, '/api-order/rest/merchant/orderDetail/list', {"pageNo" : -1, "pageSize" : 20, "orderId" : id},
							(vue, data) => {
			...
			getList(id) { // 获取商品列表
				...
				vue.$http.post(vue, '/api-order/rest/merchant/orderDetail/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/api-order/rest/merchant/orderDetail/delete', {"ids" : [id]},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/api-order/rest/merchant/orderDetail/update';						
						}else{
							url = '/api-order/rest/merchant/orderDetail/insert';	
