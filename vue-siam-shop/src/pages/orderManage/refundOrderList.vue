<template>
	<section>
      ...
      <el-table-column prop="refundCreateTime" label="申请退款时间" :formatter="formatTime" width="190"></el-table-column>
          ...
          <el-button size="small" v-if="scope.row.refundType==2 && (scope.row.refundStatus==1 || scope.row.refundStatus==2)" type="primary" @click="handleCheck(scope.row)">处理</el-button>
...
<script>
	export default {
      ...
      dealOption() {
        ...
        vue.dealtype ? url = '/api-order/rest/merchant/order/updateOrderByBack' : url = '/api-order/rest/merchant/order/updateByDealadvise'
        vue.$http.post( vue, url, param,
          (vue, data) => {
            vue.$message({
              showClose: true,
      ...
      addStand() { // 新增规格
        ...
        vue.$http.post( vue, '/api-goods/rest/merchant/stand/update', param,
          (vue, data) => {
            vue.$message({
              showClose: true,
              message: data.message,
      ...
      getList(pageNoParam) { // 获取列表
        ...
        vue.listLoading = true;
				vue.$http.post(vue, '/api-order/rest/merchant/order/afterSalesList', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				let vue = this;
				vue.$http.post(vue, '/api-goods/rest/merchant/stand/deleteById', {id},
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
        ...
        let param = {
          ...
          status: 1
				...
				vue.$http.post(vue, '/api-order/rest/merchant/order/updateOrderState', param,
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
