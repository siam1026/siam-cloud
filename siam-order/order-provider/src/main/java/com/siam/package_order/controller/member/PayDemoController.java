...
import com.siam.package_common.entity.AlipayBean;
import com.siam.package_common.entity.AlipayConfig;
import com.siam.package_common.entity.BasicData;
import com.siam.package_common.entity.BasicResult;
import com.siam.package_common.rabbitmq_config.ExchangeConfig;
import com.siam.package_common.rabbitmq_config.RabbitMqConfig;
...
public class PayDemoController {
    ...
    @Autowired
    private WxpayConfigDemo wxpayConfigDemo;
    ...
    @Autowired
    private GoodsSpecificationFeignApi goodsSpecificationFeignApi;
...
//        boolean signVerified = AlipaySignature.rsaCheckV1(params, alipayConfig.alipay_public_key, alipayConfig.charset, alipayConfig.sign_type); //调用SDK验证签名
        ...
        if(signVerified) {//验证成功
            ...
            if (trade_status.equals("TRADE_SUCCESS")){
                ...
                for(OrderDetail orderDetail : list){
                    // 修改订单状态为--已付款
                    Order updateOrderTemp = new Order();
                    updateOrderTemp.setId(order.getId());
                    updateOrderTemp.setStatus(Quantity.INT_1);
                    orderService.updateByPrimaryKeySelective(updateOrderTemp);
                    ...
                    goodsSpecificationFeignApi.decreaseStock(Integer.valueOf(goodsSpecificationId), number);*/
                }
    ...
    public BasicResult wxpay(WxpayBean wxpayBean, HttpServletRequest request, WxPayUnifiedOrderRequest wxreq){
        ...
        try {
            ...
            wxreq.setAppid(wxpayConfigDemo.getAppId());
            wxreq.setMchId(wxpayConfigDemo.getMchId());
            wxreq.setNotifyUrl(wxpayConfigDemo.getNotifyUrl());
            wxreq.setTradeType(wxpayConfigDemo.getTradeType());
    ...
    public BasicResult orderPay(WxpayBean wxpayBean, HttpServletRequest request){
        ...
        insertMemberTradeRecord.setType(Quantity.INT_1);
        insertMemberTradeRecord.setStatus(Quantity.INT_1);
        insertMemberTradeRecord.setType(Quantity.INT_0);
        /*insertMemberTradeRecord.setRemark(null);*/
        insertMemberTradeRecord.setCreateTime(new Date());
        ...
        BigDecimal purchaseRewardPoints = setting.getPurchaseRewardPoints();

        //获取用户当前积分数
        Member dbMember = memberFeignApi.selectByPrimaryKey(dbOrder.getMemberId()).getData();
        BigDecimal currentPointsNum = dbMember.getPoints();

        //获取最后的积分数
        Integer goodsNum = dbOrder.getGoodsTotalQuantity();
        goodsNum = goodsNum == null ? 0 : goodsNum;
        BigDecimal rewardPointsTotal = purchaseRewardPoints.multiply(BigDecimal.valueOf(goodsNum));
        BigDecimal pointsNnm = currentPointsNum.add(rewardPointsTotal);
        ...
        memberBillingRecord.setNumber(rewardPointsTotal);
